...
import org.apache.spark.sql.types.{ArrayType, BooleanType, LongType, StringType, StructField, StructType, TimestampType}

object SchemaFactory {

  def kafkaStreamSchema() : StructType =
    {
      new StructType()
        .add("Events", ArrayType(
          this.kafkaMessageSchema())
        )
    }

  def kafkaMessageSchema() : StructType =
  {
    new StructType()
